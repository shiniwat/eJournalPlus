...
namespace SiliconStudio.Meet.EjpLib.BaseClasses
{
	...
	public sealed class ejpStudy : IDisposable
	{
        ...
        private ObservableCollection<ejpReportNV> _reportsNV;
        public ObservableCollection<ejpReportNV> ReportsNV
        {
            get { return _reportsNV; }
            set { _reportsNV = value; }
        }
            ...
            Guid xpsDocumentId, string xpsDocumentTitle)
		{
            ...
            this._reportsNV = new ObservableCollection<ejpReportNV>();
        ...
        /// <summary>
        /// Import a nonVisual Report to the Study.
        /// </summary>
        internal void ImportReportNV(ejpReportNV report)
        {
            try
            {
                if (this._reportsNV == null)
                    this._reportsNV = new ObservableCollection<ejpReportNV>();
                if (this._reports == null)
                    this._reports = new ObservableCollection<ejpReport>();

                if(report == null)
                    throw new Exception("Attempt to import report that is null.");

                if (this._reportsNV.Count == 2 || this._reports.Count == 2 || (this._reportsNV.Count + this._reports.Count) == 2)
                    throw new Exception("No more than 2 Reports are allowed.");

                this._reportsNV.Add(report);
            }
            catch (Exception)
            {
                throw new ApplicationException("Failed to import the ReportNV object to the Study.");
            }
        }
