	...
	public partial class KnowledgeMap : System.Windows.Controls.UserControl
	{
		...
		public KnowledgeMapInputState InputState
		{
			...
			set 
			{
                ...
                "Setting KM InputState",
                "Current State: " + this._inputState.ToString() +
                "\nNew State: " + value.ToString()
                );
		...
		private void OnMapCanvasPreviewMouseUp(object sender, MouseEventArgs e)
		{
            ...
            switch (this._inputState)
            {
                ...
                case KnowledgeMapInputState.Note:
                    ...
                    this.AddTextEntity("No Title", "", new XpsDocumentReference(), "",
            ...
            if (u3 != null)
            {
                if (u3 != this._currentlyHoveringUiElement)
                {
                    if (this._currentlyHoveringUiElement != null)
                        this._currentlyHoveringUiElement.BitmapEffect = null;
                    u3.BitmapEffect = new DropShadowBitmapEffect() { ShadowDepth = 10, Color = Colors.Black, Softness = 7 };
                ...
                foreach (UIElement uie in this._IC_MapCanvas.Children)
                {
                    ...
                    catch (Exception ex)
                    {
						SiliconStudio.DebugManagers.DebugReporter.Report(
							...
							"EjpControls - Knowledge Map",
							"Hittesting Entities failed" +
							"\nParent Study ID: " + this.ParentStudyId.ToString() +
							"\nKnowledge Map ID: " + this._localMapObject.Id +
							"\nError: " + ex.Message);
                    }
                }
            }
        ...
        private void OnMapCanvasDrop(object sender, DragEventArgs e)
        {
            ...
            if (isReferencedImage)
            {
                ...
                {
                    MessageBox.Show("このナレジマップは今ロックされていますので、新しい項目を追加出来ません。", 
                        "ナレッジマップがロックされている。", MessageBoxButton.OK, MessageBoxImage.Stop);
                    return;
                }
                ...
                this.AddImageEntity("名称未設定", dropImage.IBrush.ImageSource, new SolidColorBrush(newColor),
            ...
            else if (isReferencedText)
            {
                ...
                this.AddTextEntity("名称未設定", q.UnicodeString, q.Reference, q.CommentString,
            ...
            double width, double height, double x, double y)
		{
			...
			catch (Exception ex)
			{
                ...
                SiliconStudio.DebugManagers.DebugReporter.Report(
                         SiliconStudio.DebugManagers.MessageType.Error,
                         "ejpControls - KnowledgeMap",
                         "Failed to Import Guide to KnowledgeMap" +
                         "\nError: " + ex.Message);

                throw new ApplicationException("選択された画像をガイドにすることが出来ませんでした。");
			}
		}
        ...
        public void ClearKnowledgeMapGuide()
        {
            ...
            catch (Exception ex)
            {
                SiliconStudio.DebugManagers.DebugReporter.Report(
                         SiliconStudio.DebugManagers.MessageType.Error,
                         "ejpControls - KnowledgeMap",
                         "Failed to Delete Guide from KnowledgeMap" +
                         "\nError: " + ex.Message);

                throw new ApplicationException("ガイドを削除することが出来ませんでした。");
            }
        }
                ...
                this._knowledgeMapGuideDataStream.CanRead == false)
            {
                ...
                else
                {
                    ...
                    if (bmpsource != null)
                    {
                        ...
                        throw new InvalidCastException("Guide Image source stream is not accessable. The source is not a BitmapImage/Uri.");
                }
            }
		...
		public void ImportMapObject(EjpLib.BaseClasses.ejpKnowledgeMap map)
        {
            ...
            foreach (EjpLib.BaseClasses.ejpKMTextEntity kmte in map.TextEntities)
            {
                ...
                catch (System.ArgumentException ex)
                {
					SiliconStudio.DebugManagers.DebugReporter.Report(
							SiliconStudio.DebugManagers.MessageType.Error,
							"EjpControls - Knowledge Map",
							"Importing KM Text Object Failed. Unknwon Object type. Resetting to default type." +
							"\nParent Study ID: " + map.ParentStudyId.ToString() +
							"\nKnowledge Map ID: " + map.Id.ToString() +
							"\nError: " + ex.Message);
            ...
            foreach (EjpLib.BaseClasses.ejpKMImageEntity kmi in map.ImageEntities)
            {
                ...
                catch (System.ArgumentException ex)
                {
					SiliconStudio.DebugManagers.DebugReporter.Report(
							SiliconStudio.DebugManagers.MessageType.Error,
							"EjpControls - Knowledge Map",
							"Importing KM Image Object Failed. Unknwon Object type. Resetting to default type." +
							"\nParent Study ID: " + map.ParentStudyId.ToString() +
							"\nKnowledge Map ID: " + map.Id.ToString() +
							"\nError: " + ex.Message);
				...
				//We have a try clause here to make sure an invalid Uri doesn't
				//stop us from loading an assignment.
				//BitmapImage bpi = null;
				//try
				//{
				//    Uri iUri = new Uri(kmi.ImageSource);
				//    //bpi = new BitmapImage(iUri);
				//    bpi.CacheOption = BitmapCacheOption.OnLoad;
				//}
				//catch (Exception ex)
				//{
				//    SiliconStudio.DebugManagers.DebugReporter.Report(
				//         SiliconStudio.DebugManagers.MessageType.Error,
				//         "ejpControls - KnowledgeMap",
				//         "Broken Image Entity discovered." +
				//         "\nUri: " + kmi.ImageSource + 
				//         "\nError: " + ex.Message);
				//}
				//if (bpi != null)
				//{
				//    //ie._I_ImageArea.Source = bpi;
				this._IC_MapCanvas.Children.Add(ie);
				this._nonShapeEntities.Add(ie);
				//}
            }
            ...
            foreach (EjpLib.BaseClasses.ejpKMConnectedStroke kmcs in map.ConnectedStrokes)
            {
                ...
                catch (System.ArgumentException ex)
                {
					SiliconStudio.DebugManagers.DebugReporter.Report(
							SiliconStudio.DebugManagers.MessageType.Error,
							"EjpControls - Knowledge Map",
							"Importing KM Connected Stroke Failed. Unknwon Arrow type. Resetting to default type." +
							"\nParent Study ID: " + map.ParentStudyId.ToString() +
							"\nKnowledge Map ID: " + map.Id.ToString() +
							"\nError: " + ex.Message);
            ...
            //Dictionary<Guid, SolidColorBrush> commenterColorDictionary =
            //    new Dictionary<Guid, SolidColorBrush>();

            //Random r = new Random((int)DateTime.Now.Ticks);
            foreach (EjpLib.BaseClasses.ejpCAComment comment in map.Comments)
            {
                //this is to make sure that all comments by any user ge the same color
                //and that the color is different for every user.
                ...
                if (comment.Messages != null) //== empty comment box..
                {
                    ...
                    {
                        //if (!commenterColorDictionary.ContainsKey(message.AuthorId))
                        //    commenterColorDictionary.Add(
                        //        message.AuthorId, new SolidColorBrush(
                        //            Color.FromArgb(
                        //            100, (byte)r.Next(0, 255), (byte)r.Next(0, 255), (byte)r.Next(0, 255))
                        //            ));
            ...
            //commenterColorDictionary.Clear();
		}
		...
		public void ExportMapToImage(string path)
        {
            ...
            catch (Exception ex)
            {
				SiliconStudio.DebugManagers.DebugReporter.Report(
							SiliconStudio.DebugManagers.MessageType.Error,
							"EjpControls - Knowledge Map",
							"Exporting Knowledge Map to Image file Failed" +
							"\nParent Study ID: " + this.ParentStudyId.ToString() +
							"\nKnowledge Map ID: " + this._localMapObject.Id.ToString() +
							"\nError: " + ex.Message);
            }
        }
        ...
        public string ExportMapToBase64String()
        {
            ...
            catch (Exception ex)
            {
				SiliconStudio.DebugManagers.DebugReporter.Report(
							SiliconStudio.DebugManagers.MessageType.Error,
							"EjpControls - Knowledge Map",
							"Exporting Knowledge Map to Base64 string Failed" +
							"\nParent Study ID: " + this.ParentStudyId.ToString() +
							"\nKnowledge Map ID: " + this._localMapObject.Id.ToString() +
							"\nError: " + ex.Message);
        ...
        public byte[] ExportMapToByteArray()
        {
            ...
            catch (Exception ex)
            {
				SiliconStudio.DebugManagers.DebugReporter.Report(
							SiliconStudio.DebugManagers.MessageType.Error,
							"EjpControls - Knowledge Map",
							"Exporting Knowledge Map to Byte Array Failed" +
							"\nParent Study ID: " + this.ParentStudyId.ToString() +
							"\nKnowledge Map ID: " + this._localMapObject.Id.ToString() +
							"\nError: " + ex.Message);
		...
		public void ExportMapObject()
		{
            ...
            if (this._localMapObject.Comments != null) //this is basically a version check. (if value is null, this file was created before EJP supported CA) 
            {
                ...
                foreach (KnowledgeMapComment comment in this._commentsList)
                {
                    ...
                    EjpLib.BaseClasses.ejpCAComment com = new SiliconStudio.Meet.EjpLib.BaseClasses.ejpCAComment()
                    {
        ...
        private void _CMS_InputMove(Point position)
        {
            if (this._isDraggingComment && this._currentlyDraggedComment != null)
            {
                ...
                {
                    Canvas.SetLeft(
                        this._currentlyDraggedComment, position.X - this._commentDragPositionOffset.X);
                }
                if (newY > 0 && newY < (this._cv_CommentSurface.Height - 26))
                {
                    Canvas.SetTop(
                        this._currentlyDraggedComment, position.Y - this._commentDragPositionOffset.Y);
                }
            }
        }
        ...
        private void InsertNewComment(Point origin)
        {
            try
            {
                ...
                comment.OnDeleteComment += new DeleteKnowledgeMapComment(OnCommentDeleted);
            }
            catch (Exception ex)
            {
                SiliconStudio.DebugManagers.DebugReporter.Report(
							SiliconStudio.DebugManagers.MessageType.Error,
							"EjpControls - Knowledge Map",
							"Insert new Knowledge Map Comment failed" +
							"\nParent Study ID: " + this.ParentStudyId.ToString() +
							"\nKnowledge Map ID: " + this._localMapObject.Id.ToString() +
							"\nError: " + ex.Message);
                throw;
