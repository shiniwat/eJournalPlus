/*
 * This is basically a modified version of the classic
 * Flow Document to Html converter provided by the 
 * 'Windows Presentation Foundation SDK writers and editors'
 * When this sample was written, Flowdocument's support for 
 * inline Ui elements was limited, and this converter 
 * originally did not support Hyperlinks.
 * 
 * You can find the original Blog Post about this converter at
...
namespace SiliconStudio.Meet.EjpControls.Helpers
{
    ...
    internal static class FlowDocToHTMLConverter {
        ...
        private static void WriteElementContent(XmlTextReader xamlReader, XmlTextWriter htmlWriter, StringBuilder inlineStyle)
        {
            ...
            {
                //20071205 - quick hack to support Hyperlinks..
                string navigateUri = "";
                bool insertHyperlinkRef = false;
                if (xamlReader.Name == "Hyperlink")
                {
                    navigateUri = xamlReader.GetAttribute("NavigateUri");
                    if(navigateUri != "")
                        insertHyperlinkRef = true;
                }

                while (ReadNextToken(xamlReader) && xamlReader.NodeType != XmlNodeType.EndElement)
                {
                    if (insertHyperlinkRef)
                    {
                        if (htmlWriter != null && !elementContentStarted && inlineStyle.Length > 0)
                            htmlWriter.WriteAttributeString("HREF", navigateUri);
                        insertHyperlinkRef = false;
                    }
        ...
        private static void WriteElement(XmlTextReader xamlReader, XmlTextWriter htmlWriter, StringBuilder inlineStyle)
        {
            ...
            else
            {
                ...
                string hyperlinkNavigateUri = null;
                ...
                switch (xamlReader.Name)
                {
                    ...
                    case "InlineUIContainer":
                        htmlElementName = "SPAN";
                        break;
                    case "Hyperlink": //Added 071205
                        htmlElementName = "A";
                        hyperlinkNavigateUri = xamlReader.GetAttribute("NavigateUri");
                        break;
                    case "Bold":
                        htmlElementName = "B";
                        break;
